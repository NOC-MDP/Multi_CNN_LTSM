...
class StreamingBifurcationDetector:
    ...
    def push(self, new_data: np.ndarray) -> list[BifurcationAlert]:
        ...
        p_raw, t_norm, per_var_timing = self._infer(window)
    ...
    def _assemble_window(self) -> np.ndarray:
        ...
        if full.shape[2] > keep:
            ...
            while (
                self._buffer
                and trimmed + self._buffer[0].shape[2] <= full.shape[2] - keep
            ):
    ...
    def _infer(self, window: np.ndarray) -> tuple[float, float, float]:
        ...
        x = x.to(self.device)
        p_pred, t_pred, per_var_timing = self.model(x)

        return p_pred.item(), t_pred.item(), per_var_timing.squeeze(0).tolist()
