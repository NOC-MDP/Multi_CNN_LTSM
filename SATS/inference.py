...
def run_changepoint_analysis(raw_stream, n_breakpoints=2, model_type="rbf"):
    """
    Runs offline changepoint detection on the full multivariate stream.

    Args:
        raw_stream:    np.ndarray of shape (P, T) — raw unscaled data
        n_breakpoints: how many breakpoints to search for.
                       Use 2 to capture both the onset and end of a
                       transition plateau (recommended for comparing
                       against a probability-threshold predictor).
    ...
    Returns:
        dict with breakpoint locations, per-segment means, and a confidence
        metric, or None if unavailable
    """
    ...
    # predict(n_bkps=k) returns k+1 indices, with the final entry always == T.
    # The actual changepoints are everything before that.
    transition_points = breakpoints[:-1]

    # Build segment means/lengths across all (n_breakpoints + 1) segments
    segment_bounds = [0] + transition_points + [T]
    segment_means, segment_lens = [], []
    for i in range(len(segment_bounds) - 1):
        seg = signal[segment_bounds[i]:segment_bounds[i + 1]]
        segment_means.append(seg.mean(axis=0))
        segment_lens.append(len(seg))

    # Confidence: variance of a piecewise-constant reconstruction
    # (segment means) relative to total variance — generalises the
    # original pre/post ratio to N segments
    reconstructed = np.vstack([
        np.full((seg_len, signal.shape[1]), seg_mean)
        for seg_len, seg_mean in zip(segment_lens, segment_means)
    ])
    total_var = np.var(signal, axis=0).mean()
    between_var = np.var(reconstructed, axis=0).mean()
    ...
    print(f"    Breakpoints at  : {transition_points}")
    print(f"    Confidence score: {confidence:.3f}  (between/total variance ratio)")

    return {
        "breakpoint_timesteps": transition_points,
        "confidence": confidence,
        "model_type": model_type,
        "segment_means": [m.tolist() for m in segment_means],
    }
...
def _plot_inference_result(raw_stream, processed_stream, event, cpa_result, channel_names, window_size,ensemble:int):
    ...
    for i, ax in enumerate(axes):
        ...
        if cpa_result is not None:
            ax.axvline(cpa_result["breakpoint_timesteps"][0],
                       color='forestgreen', linewidth=1.8, linestyle=':')
            ax.axvline(cpa_result["breakpoint_timesteps"][1],
                       color='forestgreen', linewidth=1.8, linestyle=':')
        ...
        if i == 0:
            ...
            if cpa_result is not None:
                ...
                handles.append(
                    plt.Line2D([0], [0], color='forestgreen', linewidth=1.8, linestyle=':',
                               label=f'CPA breakpoint  conf={cpa_result["confidence"]:.2f}  '
                                     f't={cpa_result["breakpoint_timesteps"][1]}')
                )
            if handles:
...
if __name__ == "__main__":
    ...
    for i in range(ensemble_st-1,ensemble_end,1):
        ...
        CSV_INPUT_PATH = f"ensembles/sat_obs_1_LON_-56.506_LAT_60.819.csv"
